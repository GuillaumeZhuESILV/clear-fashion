...
async function connectToDatabase() {
    try {
      ...
      price=50
      const results = await collection.find({ price: { $lt: 50 } }).toArray();
      //console.log(results);
      ...
      const less_2weeks_products=await collection.find({date: {$lt: new Date(Date.now() - (14 * 24 * 60 * 60 * 1000))}}).toArray();
